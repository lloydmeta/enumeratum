...
object Forms {
  ...
  /**
    ...
    */
  def enum[A <: EnumEntry](enum: Enum[A],
                           insensitive: Boolean = false): Mapping[A] =
  ...
  /**
    ...
    * @param insensitive bind in a case-insensitive way, defaults to false
    */
  def format[A <: EnumEntry](enum: Enum[A],
                             insensitive: Boolean = false): Formatter[A] =
    new Formatter[A] {
      def bind(key: String, data: Map[String, String]) = {
        play.api.data.format.Formats.stringFormat
          .bind(key, data)
          .right
          .flatMap { s =>
            val maybeBound =
