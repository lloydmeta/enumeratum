...
object EnumHandler {
  ...
  def reader[A <: EnumEntry](
      enum: Enum[A],
      insensitive: Boolean = false): BSONReader[BSONValue, A] =
  ...
  def readerLowercaseOnly[A <: EnumEntry](
      enum: Enum[A]): BSONReader[BSONValue, A] =
    new BSONReader[BSONValue, A] {
      override def read(bson: BSONValue): A = {
        bson match {
  ...
  /**
    ...
    */
  def writerLowercase[A <: EnumEntry](
      enum: Enum[A]): BSONWriter[A, BSONValue] = new BSONWriter[A, BSONValue] {
  ...
  /**
    ...
    */
  def writerUppercase[A <: EnumEntry](
      enum: Enum[A]): BSONWriter[A, BSONValue] = new BSONWriter[A, BSONValue] {
  ...
  def handler[A <: EnumEntry](
      enum: Enum[A],
      insensitive: Boolean = false): BSONHandler[BSONValue, A] =
  ...
  def handlerLowercaseOnly[A <: EnumEntry](
      enum: Enum[A]): BSONHandler[BSONValue, A] =
    new BSONHandler[BSONValue, A] {
